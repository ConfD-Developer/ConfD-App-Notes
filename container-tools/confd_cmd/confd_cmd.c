...
static void do_suspend2(char *argv[])
{
    int s;
    pid_t pid;
    assert((s = get_socket(ai)) >= 0);
    OK(cdb_connect(s, CDB_DATA_SOCKET, ai->ai_addr, ai->ai_addrlen));
    start_session(s, db);
    if ((pid = fork()) == 0) {
        /* child */
...
static void do_maapi_get_attrs(char *argv[])
{
    ...
    int nvals;
    u_int32_t attrs[] = { confd_str2hash(argv[1]) };
    int num_attrs = atoi(argv[2]);

    OK(maapi_get_attrs(ms, mtid, attrs, num_attrs, &attrvals, &nvals, argv[0]));
}
...
#define ZERO_OR_MORE_ARGS (-(MAX_ARGS-1))
    ...
    {
        "maapi_get_attrs", {"mget_attrs",NULL}, do_maapi_get_attrs, 3,
        CMD_MAAPI|CMD_WANT_SCHEMA, "<path>, <attrs>, <num_attrs>", NULL
    },
    ...
    for (pc=pgm->pgm; pc; pc=pc->next) {
        ...
        if (cc->cmd) {
            ...
            if ((cc->cmd_flags & CMD_CDB) && (cs < 0)) {
                ...
                assert((cs = get_socket(ai)) >= 0);
                OK(cdb_connect(cs, st, ai->ai_addr, ai->ai_addrlen));
            ...
            if (cc->cmd_flags & CMD_MAAPI) {
                ...
                if (ms < 0) {
                    ...
                    OK(maapi_connect(ms, ai->ai_addr, ai->ai_addrlen));
            ...
            if ((cc->cmd_flags & CMD_HA) && (hs < 0)) {
                assert((hs = get_socket(ai)) >= 0);
                OK(confd_ha_connect(hs, ai->ai_addr, ai->ai_addrlen, progname));
            }
            cc->func(pc->argv + 1);
...
static void do_listen(int port)
{
    ...
    if ((pid = fork()) == 0) {
        ...
        if ((ctlsock = get_socket(ai)) < 0)
            confd_fatal("Failed to open ctlsocket\n");
        if (confd_connect(dctx, ctlsock, CONTROL_SOCKET, ai->ai_addr, ai->ai_addrlen) < 0)
...
int main(int argc, char *argv[])
{
    ...
    /* Parse command line */
    while ((c = getopt(argc, argv, "da:p:orSIf:isUu:x:g:etc:l:mhL?")) != EOF) {
        switch (c) {
        case 'd':
            debug++;
            break;
    ...
    ai = lookup(confd_addr, confd_port);

    /* always save trace output when testing */
    if ((debug == CONFD_SILENT) && (is_env_set("TEST_DIR"))) {
        char fname[255];
        char *suffix = getenv("CONFD_CMD_TRACE_SUFFIX");
        char *mode;
        struct sockaddr_in *in_addr_p = (struct sockaddr_in *)ai->ai_addr;

        if ((ai->ai_family != PF_INET) || (ntohs(in_addr_p->sin_port) == PORT)) {
            snprintf(fname, sizeof(fname), "_tmp_%s", progname);
        } else {
            snprintf(fname, sizeof(fname), "_tmp_%s.%s", progname, confd_port);
        }
        if (suffix) {
            char tmpstr[16];
