...
#endif
        "    -F x|p|o|j|c|i|t format (one of x(ml), p(retty xml), json or\n"
        "                  j, c, i, t(turbo c) (-style cli)) [default is xml]\n"
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "bxnNdjF:WSp:P:f:lmrHUaeoMDiu:g:c:tsOh?"))
           != -1)
    {
        switch(c) {
        case 'x':
